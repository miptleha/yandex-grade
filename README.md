# Обзор Яндекс Грейд

Корпоративное решение для прокачки навыков сотрудников.  
В отличии от [Яндекс Практикума](https://practicum.yandex.ru) (каждый курс в котором длится от нескольких месяцев до нескольких лет), курсы в [Яндекс Грейде](https://practicum.yandex.ru/grade) предельно короткие (от нескольких минут до нескольких часов).  
Есть ли что-то полезное в таких коротких курсах? Давайте разбираться.

## Тесты под роль

Так называется раздел в `Грейде`, в котором собраны тесты проверки профессиональных навыков. С этого раздела логично начать, чтобы самопровериться.

* Автоматизатор тестирования на Python, *20 минут* :boy: 58% (24.10.24)
* Автоматизатор тестирования на Java, *40 минут* :boy: 52% (24.10.24)
* Алгоритмы и структуры данных, *28 минут* :boy: 61% (25.10.24)
* Аналитик данных, *3 часa* :boy: 67% (25.10.24)
* Бизнес-аналитик, *28 минут* :man: 68% (25.10.24)
* Дизайнер в IT, *24 минуты* :boy: 54% (25.10.24)
* Интернет-маркетолог, *52 минуты* :boy: 51% (25.10.24)
* Менеджер проектов, *2 часa* :boy: 75% (25.10.24)
* Начинающий продакт-менеджер, *24 минуты* :boy: 60% (25.10.24)
* Продакт-менеджер с опытом, *32 минуты* :man: 57% (27.10.24)
* Управление командой, *58 минут* :boy: 54% (27.10.24)
* Фронтенд-разработчик, *18 минут* :boy: 77% (27.10.24)
* DevOps-инженер, *34 минуты* :boy: 69% (27.10.24)
* Go-разработчик, *24 минуты* :man: 46% (27.10.24)
* Java-разработчик, *18 минут* :boy: 73% (27.10.24)
* HR-менеджер, *1 час* :boy: 56% (28.10.24)
* Python-разработчик, *30 минут* :boy: 45% (28.10.24)
* Middle Python-разработчик, *16 минут* :man: 48% (28.10.24)

Каждый тест состоит из 5-15 блоков по 1-4 вопроса. Прохождение одного теста занимает не более 15 минут.  
Тут не столько результат теста важен (их завышают), а интерес к темам теста.  
Можно быстро понять, что не вызывает зевоту и чем интересно было бы заниматься.

П.С. Этот раздел был удален спустя месяц, тесты переместили в соответсвующие разделы.

## Английский для работы

Время прокачать свой иностранный язык. `Грейд` любезно предоставляет обучающие уроки по английскому.

* Connecting with the team (Построение отношений с новыми коллегами), *2 часа* :man: 49% (29.10.24)
* Giving a presentation (Построение структуры публичного выступления), *2 часа* :man: 57% (29.10.24)
* Participating in a discussion (Обсуждение идей и решений в команде), *2 часа* :man: 57% (29.10.24)
* Performance review (Презентация своих достижений и планов развития), *37 минут* :man: 25% (30.10.24)
* Sync-up meetings (Обсуждение завершённых, текущих и запланированных задач), *2 часа* :man: 29% (30.10.24)

Тяжеловато для меня, многие выражения на английском не знакомы.  
В каждом уроке имеется небольшая глава с теорией и итоговый тест. Каждый урок минут на 20.

## Алгоритмы

Перед тем, как приступить к самой интересной теме, прокачаем свою `computer science`.

* Знакомство с алгоритмами, *5 часов* :boy: 34% (30.10.24) [Контест](https://contest.yandex.ru/contest/59191/problems/)
  - [x] Решение базовых алгоритмических задач
* Введение в алгоритмы, *9 часов* :boy: 79% (01.11.24) [Контест](https://contest.yandex.ru/contest/59194/problems/)
  - [x] Решение базовых алгоритмических задач
  - [x] Оценка сложности алгоритмов и сравнение их по эффективности
* Основные структуры данных, *8 часов* :boy: 89% (03.11.24) [Контест](https://contest.yandex.ru/contest/59182/problems/)
  - [x] Работа со списками
  - [x] Работа с массивами
  - [x] Работа с очередью
  - [x] Работа со стеком
* Рекурсия и сортировки, *13 часов* :man: 90% (06.11.24) [Контест](https://contest.yandex.ru/contest/59639/problems/)
  - [x] Решение алгоритмических задач с помощью рекурсии
  - [x] Применение алгоритмов сортировки
* Работа с деревьями, *11 часов* :man: 65% (11.11.24) [Контест](https://contest.yandex.ru/contest/60070/problems/)
  - [x] Работа с деревьями
* Хеш-функции и хеш-таблицы, *10 часов* :man: 95% (12.11.24) [Контест](https://contest.yandex.ru/contest/59971/problems/)
  - [x] Работа с хеш-функциями и хеш-таблицами
* Работа с графами, *11 часов* :man: 72% (15.11.24) [Контест](https://contest.yandex.ru/contest/59363/problems/)
  - [x] Работа с графами
* Жадные алгоритмы и динамическое программирование, *9 часов* :man: 77% (23.11.24) [Контест](https://contest.yandex.ru/contest/59565/problems/)
  - [x] Решение алгоритмических задач с помощью динамического программирования
  - [x] Применение жадных алгоритмов для решения задач

На примере простых алгоритмов показано, как можно их оптимизировать.  
Уроки теоритические, но встречаются ссылки на `Яндекс Контест`, где на практике можно применить описываемые алгоритмы.  
Сначала решал на `C#`, но потом проникся духом `Python` и стал на нем решать.  
Каждый курс вместе с задачками займет от одного до нескольких дней, в зависимости от сложности курса.  
Столкнулся с тем, что тесты по курсу нельзя пересдать. Во время финального теста нельзя отвлекаться.  
Самые сложные темы - графы и динамическое программирование, в основном пропустил их, очень сложно.

## Менеджемент

После алгоритмов надо отдохнуть. Прокачаем свои `soft skills`.  
Тут куча грейдов по управлению, быстро просмотрю все.

* Управление проектами
  - [x] Решение проблем, *2 часa* :boy: 28% (23.11.24)
  - [x] Переговоры, *2 часa* :boy: 57% (24.11.24)
  - [x] Готовимся к встрече с клиентом, *4 часa* :boy: 74% (25.11.24)
  - [x] Продуктовая разработка, *5 часов* :boy: 22% (25.11.24)
  - [x] Оценка, планирование, смета, *6 часов* :boy: 32% (25.11.24)
  - [x] Гибкие методологии и организация работ в команде, *7 часов* :boy: 45% (25.11.24)
  - [ ] Предпроектная аналитика, *7 часов* :boy:
  - [ ] Работаем с планом и требованиями, *14 часов* :boy:
  - [ ] Проектное управление. Этап финализации работ, *6 часов* :man:
  - [ ] Дизайн и разработка, *8 часов* :man:
  - [ ] Планирование, *9 часов* :man:
  - [ ] Разработка и релиз мобильных приложений для менеджера IT-проектов, *11 часов* :man:
  - [x] Основы проджект-менеджмента, *26 часов* :man: 46% (26.11.24)

К сожалению не особо разбираюсь в теме. Часть пролистал, часть даже не открывал.   
Не буду выписывать и проходить кучу курсов по менеджементу (**Управление продуктом**, **Управление финансами**, **Управление командой**, **HR-аналитика**, **HR-менеджемент**, **Стратегия и анализ рынка**). Разве что один интересный раздел.  

* Мягкие навыки руководителя
  - [x] Алгоритмы бесконфликтного общения, *4 часa* :boy: 87% (27.11.24)
  - [x] Планирование рабочей коммуникации, *4 часa* :boy: 73% (27.11.24)
  - [x] Навыки аргументации, *5 часов* :boy: 48% (27.11.24)
  - [ ] Приоритеты и делегирование, *6 часов* :boy:
  - [ ] Креативное мышление для поиска решений, *6 часов* :boy:
  - [ ] Как увлечь слушателей, *6 часов* :boy:
  - [x] Как говорить убедительно, *6 часов* :boy: 85% (27.11.24)
  - [ ] Как провести эффективное совещание, *6 часов* :boy:
  - [x] Критическое мышление: анализ информации и принятие решений, *6 часов* :boy: 84% (28.11.24)
  - [x] Основы логики для принятия решений, *7 часов* :boy: 60% (28.11.24)
  - [x] Сложные переговоры, *4 часa* :man: 71% (29.11.24)
  - [x] Собираем команду и ставим задачу, *6 часов* :man: 72% (29.11.24)
  - [x] Наставничество: как выращивать самостоятельных специалистов, *9 часов* :boy: 68% (01.12.24)
  - [ ] Планирование коммуникации, *4 часa* :man:
  - [ ] Коммуникация в проекте, *6 часов* :man:
  - [ ] Управление коммуникацией в IT-командах, *7 часов* :man:
  - [x] Основы аргументации. Структура мысли, *5 часов* :man: 88% (01.12.24)
  - [ ] Личное развитие тимлида разработчиков, *8 часов* :man:
  - [ ] Развитие команды и продукта, *8 часов* :man:
  - [ ] Начало работы с IT-командой и продуктом, *11 часов* :man:
  - [ ] Работа со специалистом один на один, *12 часов* :man:
 
Нет времени, чтобы изучить все. Пришлось часть пропустить.

## Дизайн

К сожалению, вынужден буду пропустить, хотя тема очень интересная. Иначе не успею прочитать про разработку.  
Кратко отмечу основные грейды для этого раздела.

* Основы дизайна в ИТ
  - [ ] Цвет в работе дизайнера, *4 часa* :boy:
  - [ ] Визуальные презентации, *6 часов* :boy:
  - [ ] Сетки в графическом и веб-дизайне, *7 часов* :boy:
  - [ ] Основы композиции в дизайне, *7 часов* :boy:
  - [ ] Базовая типографика, *17 часов* :boy:
  - [x] Основы Figma, *19 часов* :boy: 76% (07.12.2024)
* Дизайн интерфейсов
  - [ ] Как собрать портфолио, *3 часa* :boy:
  - [ ] CJM, Jobs To be Done и работа с гипотезами, *4 часa* :boy:
  - [ ] Анимация в интерфейсе: от теории к практике, *4 часa* :man:
  - [ ] Текст в интерфейсе, *10 часов* :boy:
  - [ ] Основы UX, *11 часов* :boy:
  - [ ] Адаптация сайтов со сложными элементами, *8 часов* :man:
* Дизайн мобильных приложений
  - [ ] Основы дизайн-системы и анализ сценариев продукта, *3 часa* :man:
  - [ ] Основы дизайна интерфейсов мобильных приложений, *4 часa* :man:
  - [ ] Исследование, анализ и работа с гипотезами, *4 часa* :man:
  - [ ] Разработка и презентация проектных решений, *6 часов* :man:
* Графический дизайн
  - [ ] Визуальная коммуникация, *2 часa* :boy:
  - [ ] Основы маркетинга для дизайнеров, *2 часa* :boy:
  - [ ] Оформление соцсетей, *2 часa* :boy:
  - [ ] Портрет аудитории и мудборд, *3 часa* :boy:
  - [ ] Насмотренность для дизайнера, *3 часa* :boy:
  - [ ] Основы брендинга и фирменного стиля, *3 часa* :boy:
  - [ ] Работа с иллюстрациями, *4 часa* :boy:
  - [ ] Общение с заказчиком, *4 часa* :boy:
  - [ ] Создание брендбука, *5 часов* :boy:
  - [ ] Как делать рассылки, *6 часов* :boy:
  - [ ] Обработка фотографий в Photoshop и Figma, *6 часов* :boy:
  - [ ] Создание логотипа и фирменного знака, *7 часов* :boy:
  - [ ] Векторная графика в Adobe Illustrator, *7 часов* :boy:
  - [ ] Фотографии в работе дизайнера, *5 часов* :boy:
  - [ ] Поиск идей и креативность, *6 часов* :boy:
  - [ ] Создание лендинга, *9 часов* :boy:
  - [ ] Работа с паттернами в Adobe Illustrator, *2 часa* :man:
  - [ ] Иконки в веб-дизайне, *4 часa* :man:
  - [ ] Продвинутая типографика, *16 часов* :man:
* Продуктовый дизайн
  - [ ] Коммуникация с разработкой, *4 часa* :man:
  - [ ] Презентация результатов для продуктового дизайнера, *4 часa* :man:
  - [ ] Количественные исследования, *5 часов* :man:
  - [ ] Метрики в продуктовом дизайне, *5 часов* :man:
  - [ ] Продуктовая разработка и дизайн, *5 часов* :man:
  - [ ] Качественные исследования, *7 часов* :man:

Времени нет проходить, только курс по `Фигме` несколько дней изучал, а скоро доступ к `Грейду` закроют.

## Разработка

### C#

Для разминки пройдем мой любимый `C#`

* Основы работы с C#, *28 часов* :boy: 92% (15.12.2024)
  - [x] Массивы
  - [x] Коллекции и генерики
  - [x] Упорядоченные и частично упорядоченные списки
  - [x] Ассоциированные коллекции
  - [x] Родовые типы - дженерики
  - [x] Делегаты и лямбды
  - [x] Перегрузка и расширение методов
  - [x] Анонимные типы
  - [x] Нулевой ссылочный тип
  - [x] Работа со строками и Regex
* Асинхронность и многопоточность, *15 часов* :man: 90% (22.12.2024)
  - [x] Многопоточность
  - [x] Асинхронное программирование
  - [x] Асинхронное ожидание
  - [x] Параллельная библиотека задач
  - [x] Параллельный LINQ

Замечательные темы по `C#`, жалко что их мало. В принципе, я все это уже знал, но повторение не бывает лишним.

### Python

Мне нравится `Python`. Немного его знаю, но нет опыта использования в проектах. Просмотрю пару курсов.

* Основы программирования на Python, *22 часа* :boy: 85% (31.12.2024)
  - [x] Работа с функциями
  - [x] Использование синтаксиса в Python
  - [x] Работа с циклами и ветвлениями
  - [x] Работа со словарями и множествами
  - [x] Работа со строками и форматированием
  - [x] Работа с библиотеками
  - [x] Сетевые запросы
  - [x] Написание простых программ на Python
* Рефакторинг кода по принципам ООП, *4 часа* :man: 100% (31.12.2024)
* Ускорение выполнения программ за счёт распараллеливания, *10 часов* :man: 94% (01.01.2025)
* Основы асинхронного подхода в Python, *15 часов* :man:
* Создание веб-приложений на Flask, *18 часов* :man: 70% (05.01.2025)
* FastAPI. Расширенный курс, *28 часов* :man:

И еще куча классных курсов по `Питону`. Чувствую, как становлюсь сильнее в нем. Но времени нет, надо двигаться дальше.

### Web-разработка

Самое интересное оставил на конец. Жалко, что пара дней осталось и закроют доступ. Ну, что успею, почитаю.

* TypeScript для Web-разработчика, *10 часов* :man: 93% (08.01.2025)
  - [x] Использование interface и type для определения структур данных
  - [x] Использование кастомных и встроенных дженериков
  - [x] Работа с условными типами
  - [x] Типизация Redux
  - [x] Типизация React
* Продвинутый JS. Создание интернет-магазина, *19 часов* :man: 94% (10.01.2025)
  - [x] Асинхронное программирование
  - [x] Promise API
  - [x] Работа с API
  - [x] Разработка интернет-магазина
* ООП для Web-разработчика, *11 часов* :man: 73% (11.01.2025)
* Бэкенд на Node.js и Express, *10 часов* :man: 78% (13.01.2025)

### Заключение

Чтож все хорошее заканчивается. Спасибо `Yandex` и работодателю за прекрасные курсы.  
Конечно, хотелось бы, чтобы их было больше, особенно по программированию.  
Всем спасибо, рекомендую учиться в `Yandex Grade`.

[![hits](https://myhits.vercel.app/api/hit/https%3A%2F%2Fgithub.com%2Fmiptleha%2Fyandex-grade?color=blue&label=hits&size=small)](https://myhits.vercel.app)
